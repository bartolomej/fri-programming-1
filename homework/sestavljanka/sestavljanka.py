...
"""
    ...
    kosi_krave = [
        [('1b5HCW3', '1     3', 'y     K', 'y     4', 'D     n', 'G     o', 'wXak7Lf'),
         ('3Nar3Ot', '3     t', 'K     t', '4     y', 'n     V', 'o  ___A', 'fs8Ef8L'),
         ('tpVTC39', 't     9', 't     g', 'y     I', 'V     4', 'A_____2', 'L3HqdZB'),
         ('9IHJuv0', '9     0', 'g     Q', 'I     I', '4     E', '2_____c', 'BORyS4Y'),
         ('0FZXo7p', '0     p', 'Q     R', 'I     R', 'E     L', 'c_____S', 'YZ9jZIh'),
         ('pa8UE6z', 'p     z', 'R     o', 'R     p', 'L     O', 'S_____s', 'hEz4a2S'),
         ('z2fqiM0', 'z     0', 'o     S', 'p    /j', 'O    \\6', 's_____y', 'SAKQw9b'),
         ('0zyBXOF', '0  /; F', 'S__  \\o', 'j{_\\_/F', '6___  1', 'y_/   t', 'bWjos7X'),
         ('FgB5BpV', 'F   ;\\V', 'o\\____R', "F   `'z", '1 (o) q', 't     b', 'XJikjnl'),
         ('Vh3qiPx', 'V     x', 'R//   4', 'z\\____N', 'q (o  n', 'b  :--b', 'lOf1Uia'),
         ('xQcaqz2', 'x     2', '4     h', 'N     C', 'n}    j', "b'    w", 'ain6lCM')],
        [('wXak7Lf', 'w   ,-f', 'S  ;:(0', 'C  :: T', '7  :: r', 'z  :: 8', 'IjJSaDr'),
         ('fs8Ef8L', "f,'`@@L", '0  @@@4', 'T)  @@H', 'r: @@@x', '8\\  @@I', 'rFYzMBw'),
         ('L3HqdZB', 'L@@@@@B', '4@@@@@s', 'H@@   x', 'x@@:  S', 'I@@@: b', 'wGiN3QV'),
         ('BORyS4Y', 'B@    Y', 's@    6', 'x     4', 'S     u', 'b     1', 'VvRhZ9n'),
         ('YZ9jZIh', 'Y   @@h', '6    @o', '4  @@@5', 'u   @@b', '1 @@@@x', 'nuGacd3'),
         ('hEz4a2S', 'h@@@@ S', 'o@@   K', '5@@@  X', 'b@@   S', 'x@@@) n', '3xoqLX6'),
         ('SAKQw9b', 'S     b', 'K     M', 'X     o', 'S     2', 'n   ( j', '6r1GFJy'),
         ('bWjos7X', 'b   \\_X', 'M     v', "o ,'@@v", '2 `@@@Y', "j '@@@x", 'yQBi57m'),
         ('XJikjnl', 'X    `l', 'v\\___(H', 'v(  `=0', 'Y:    P', "x'    P", 'mvw9gEu'),
         ('lOf1Uia', 'l__\\  a', "Ho'o) N", "0===' i", 'P     O', 'P     u', 'uG1USjl'),
         ('ain6lCM', 'a     M', 'N     5', 'i     N', 'O     W', 'u     q', 'lUZd7RY')],
        [('IjJSaDr', 'I  ;; r', 'o  ::/i', "O ;;'`W", 'f;;;; g', "3`'`' N", 'lz3SKAF'),
         ('rFYzMBw', 'r/\\   w', 'i  )  s', 'W; :  s', 'g: :  7', 'N/ :  4', 'FYkvB0Q'),
         ('wGiN3QV', 'w   /`V', 's  {_-f', 's )   E', '7 ;   u', '4:    z', 'QuxLObj'),
         ('VvRhZ9n', 'V,    n', 'f-----l', 'E     G', 'u     e', 'z     i', 'jfT3GWN'),
         ('nuGacd3', 'n@@@@@3', 'l-----s', 'G     3', 'e     W', 'i     R', 'N9EwL5L'),
         ('3xoqLX6', '3@@@@\\6', 's-----y', '3     U', 'W     n', 'R     w', 'LxdVORG'),
         ('6r1GFJy', '6   :@y', 'y:  :~F', 'U:  / O', 'n:  ; d', 'w:  : b', 'Gg9mkCA'),
         ('yQBi57m', 'y@@@@)m', 'F`,~~;W', 'O`; ; H', 'd ; : H', 'b : : v', 'AWa0RHL'),
         ('mvw9gEu', 'm     u', 'W     p', 'H     p', 'H     W', 'v     l', 'LkJJ05O'),
         ('uG1USjl', 'u     l', 'p     D', 'p     k', 'W     k', 'l     v', 'OTRnBf8'),
         ('lUZd7RY', 'l     Y', 'D     B', 'k     g', 'k     J', 'v     5', '8wkVWz6')],
        [('lz3SKAF', 'l    )F', '7    :P', 'g     r', 'T     h', 'D     V', '3fK3wxo'),
         ('FYkvB0Q', 'F_ \\__Q', 'P__\\  D', 'r   `^I', 'h     O', 'V     q', 'o39uUq5'),
         ('QuxLObj', 'Q;    j', 'D\\    h', "I'    z", 'O     w', 'q     F', '5az8RAa'),
         ('jfT3GWN', 'j  ";"N', "h* `,'Q", 'z \\ :/j', 'w     D', 'F     p', 'aw5UE1d'),
         ('N9EwL5L', 'N     L', 'Q*    e', 'j     X', 'D     B', 'p     I', 'dcCiv7B'),
         ('LxdVORG', 'L     G', 'e     3', 'X     r', 'B     7', 'I     w', 'BR5j6k5'),
         ('Gg9mkCA', 'G:_ ; A', '3\\  \\ s', "r `^' I", '7     5', 'w     M', '50lp8bM'),
         ('AWa0RHL', 'A \\_\\ L', 's :  \\y', 'I `-^-f', '5     Y', 'M     c', 'MqwQktc'),
         ('LkJJ05O', 'L     O', 'y   * k', "f'   \\B", 'Y     m', 'c     w', 'cyDdgEP'),
         ('OTRnBf8', "O `,',8", "k 8`;'n", 'Bv/ : I', 'm     z', 'w     e', 'PLDFqTP'),
         ('8wkVWz6', "8'    6", 'n*  * M', 'I \\/  N', 'z     X', 'e     E', 'P8kRmOP')],
    ]
...
def zrcaljen_vod(kos):
    return tuple(map(lambda x: x[::-1], kos))
...
def transpose(a):
    out = [[0 for x in range(len(a[0]))] for x in range(len(a))]
...
def obrnjen(kos):
    return zrcaljen_vod(transpose(kos))
...
def stranice(kos):
    o = obrnjen(kos)
    k = [kos[0], o[-1][::-1], kos[-1], o[0][::-1]]
    return [] + k + list(map(lambda x: x[::-1], k))
...
def obrati(kos):
    ...
    for i in range(4):
        s.add(k)
        s.add(zrcaljen_vod(k))
        s.add(zrcaljen_navp(k))
        k = obrnjen(k)
    return s
...
def preberi_datoteko(ime_datoteke):
    kosi = open(ime_datoteke, encoding='utf-8').read().split('\n\n')
    return list(map(lambda x: tuple(x.split('\n')), kosi))


def zbirka_stranic(kosi):
    vse_stranice = reduce(lambda p, c: p + c, map(lambda x: stranice(x), kosi), [])
    m = {}
    for s in vse_stranice:
        kosi_s = set()
        for k in kosi:
            if s in stranice(k):
                kosi_s.add(k)
...
def kotni(kosi, zbirka=None):
    zbirka = zbirka or zbirka_stranic(kosi)
    kotni_kosi = set()
    for kos in kosi:
        n = 0
        for s in stranice(kos)[0:4]:
            if len(zbirka[s]) == 2:
                n += 1
        # ce ima kos tocno dva roba sorodna z drugimi kosi
        # to pomeni, da ta kos lezi v vokalu
        if n == 2:
            kotni_kosi.add(kos)
    return kotni_kosi


def robni(kosi, zbirka=None):
    zbirka = zbirka or zbirka_stranic(kosi)
    robni_kosi = set()
    for kos in kosi:
        n = 0
        for s in stranice(kos)[0:4]:
            if len(zbirka[s]) == 2:
                n += 1
        # ce ima kos tocno dva roba sorodna z drugimi kosi
        # pomeni, da ta kos lezi v vokalu
        if n == 3:
            robni_kosi.add(kos)
    return robni_kosi
...
def je_ustrezna(actual, expected, po_stranicah):
    if expected == 1:
        return len(po_stranicah[actual]) == 1
    return actual == expected


def preobrni(kos, levo, gor, po_stranicah):
    set_obrati = obrati(kos)
    for mutation in set_obrati:
        zgoraj = stranice(mutation)[0]
        leva = stranice(mutation)[3]
        if je_ustrezna(zgoraj, gor, po_stranicah) and je_ustrezna(leva, levo, po_stranicah):
            return mutation
    return None


# pri argumentih levo=YYTq, gor=1
# se pojavita dva kosa, ki ustrezata tem pogojem 🤔
# to sta: ('YYTq', 'T  6', 'P  2', 'OOv5') in ('ww2F', 'J  n', 'M  y', 'YYTq')
# ki se preobrneta v ('YTPO', 'Y  O', 'T  v', 'q625') in ('YMJw', 'Y  w', 'T  2', 'qynF')
# zaradi tega posledicno ne deluje tudi funkcija sestavi_koscke
# sklepam da sem spodnjo funkcijo narobe napisal (ceprav so vsi predhodni testi zeleni)
# sem se pa malo pozno lotil in zato nisem uspel izsledit hrosca 🙃
def vzemi_kos(levo, gor, po_stranicah, uporabljeni):
    vsi_kosi = reduce(lambda p, c: p | po_stranicah[c], po_stranicah, set())
    robni_kosi = robni(vsi_kosi, po_stranicah)
    kandidati = (po_stranicah.get(levo) or set()) | (po_stranicah.get(gor) or set()) | robni_kosi
    for kos in kandidati:
        res = preobrni(kos, levo, gor, po_stranicah)
        if res and kos not in uporabljeni:
            uporabljeni.add(kos)
            return res
    return None
...
def sestavi_koscke(kosi, kos):
    uporabljeni = {kos}
    sestavljenka = [[kos]]
    zbirka = zbirka_stranic(kosi)
    while len(uporabljeni) < len(kosi):
        x = len(sestavljenka[-1])
        y = len(sestavljenka) - 1
        stranice_gor = stranice(sestavljenka[-2][x])[2] if y > 0 else 1
        stranice_levo = stranice(sestavljenka[-1][-1])[1] if x > 0 else 1
        naslednji = vzemi_kos(stranice_levo, stranice_gor, zbirka, uporabljeni)
        if not naslednji:
            sestavljenka.append([])
        else:
            sestavljenka[-1].append(naslednji)
    return sestavljenka
