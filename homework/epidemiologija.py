...
def stevilo_okuzenih(oseba):
    if oseba not in stiki:
        return 1
    return 1 + sum([stevilo_okuzenih(x) for x in stiki[oseba]])


def rekord(oseba):
    if oseba not in stiki:
        return 0
    return max([len(stiki[oseba]), max([rekord(x) for x in stiki[oseba]])])


def nevarnost_1(oseba, razlog):
    return [nevarnost(x, razlog) + razlog in stiki[oseba][x] for x in stiki[oseba]]


def nevarnost(oseba, razlog):
    if oseba not in stiki:
        return 0
    return [(razlog in stiki[oseba][x], nevarnost(x)) for x in stiki[oseba]]


def nevarnost(oseba, razlog):
    if oseba not in stiki:
        return 0
    return reduce(lambda p, c: p + c[1] + (1 if c[0] else 0),
                  [(razlog in stiki[oseba][x], nevarnost(x, razlog)) for x in stiki[oseba]], 0)


def okuzbe_zaradi(oseba, razlog):
    if oseba not in stiki:
        return 1
    return 1 + reduce(lambda p, c: p + (c[1] if c[0] else 0), [(razlog in stiki[oseba][x], okuzbe_zaradi(x, razlog)) for x in stiki[oseba]], 0)


def okuzbe_brez(oseba, prepovedane):
    if oseba not in stiki:
        return 1
    s = 0
    for o in stiki[oseba]:
        if not prepovedane or prepovedane.intersection(stiki[oseba][o]):
            s += okuzbe_brez(o, prepovedane)
        else:
            s += 1
    return s
